...
const choseMyFeeling = () => {
  (async () => {
    ...
    if (`${answer1.feeling}` === '😄') {
      const words = fs.readFileSync('quotes/a.txt', 'utf-8')
      ...
      const words = fs.readFileSync('quotes/b.txt', 'utf-8')
      const word = words.split('/')
      const phrases = word.slice(0, 14)
      const x = Math.floor(Math.random() * phrases.length)
      setTimeout(() => console.log(phrases[x]), 1000)
    } else if (`${answer1.feeling}` === '🙁') {
      const words = fs.readFileSync('quotes/c.txt', 'utf-8')
      const word = words.split('/')
      const phrases = word.slice(0, 14)
      const x = Math.floor(Math.random() * phrases.length)
      setTimeout(() => console.log(phrases[x]), 1000)
    } else {
      const words = fs.readFileSync('quotes/d.txt', 'utf-8')
      const word = words.split('/')
      const phrases = word.slice(0, 14)
      const x = Math.floor(Math.random() * phrases.length)
      setTimeout(() => console.log(phrases[x]), 1000)
...
const saveMyFeeling = () => {
  (async () => {
    ...
    fs.writeFileSync(`Myfeeling/${format}.txt`, `${format}の気分: ${answer2.feeling}`)
    ...
    if (`${answer3.confirmation}` === 'Yes') {
      ...
      reader.on('line', (input) => {
        fs.appendFile(`Myfeeling/${format}.txt`, '\n' + input, (err) => {
          if (err) throw err
